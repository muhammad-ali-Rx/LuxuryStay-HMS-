...
export default function RoomsManagement() {
  ...
  const fetchRooms = async () => {
    try {
      ...
      console.log("[v0] Fetched rooms:", roomsArray.length);
      setRooms(roomsArray);
    } catch (error) {
  ...
  const parseAmenities = (amenitiesData) => {
    ...
    if (
      ...
      amenitiesData.every((a) => typeof a === "string")
    ) {
    ...
    if (typeof amenitiesData === "string") {
      try {
        ...
        // If parsed result is still a string, parse again (handle double stringification)
        if (typeof parsed === "string") {
          return JSON.parse(parsed).filter((a) => a && a.trim());
        }
        // If it's an array, return filtered
  ...
  if (type === "edit" && room) {
    ...
    console.log("📋 Original room amenities:", room.amenities)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // FIX: Always send amenities, even if empty array
      if (formData.amenities.length > 0) {
        formData.amenities.forEach((amenity) => {
          formDataToSend.append("amenities", amenity);
        });
      } else {
        // Send empty amenities array explicitly
      ...
      if (modalType === "edit" && selectedRoom) {
        console.log(
          "[v0] Sending PUT request to update room:",
          selectedRoom._id
        );
        console.log("[v0] Amenities being sent:", formData.amenities);
      ...
      if (response.ok) {
        setSuccess(
          `Room ${modalType === "add" ? "created" : "updated"} successfully!`
        );
        setTimeout(() => {
          fetchRooms();
          setShowModal(false);
        ...
        const errorMsg =
          errorData.error || errorData.message || "Failed to save room";

        if (errorMsg.includes("E11000") || errorMsg.includes("duplicate")) {
          setError(
            `Room number ${formData.roomNumber} already exists. Please use a different room number.`
          );
        } else {
  ...
  const handleStatusChange = async (roomId, newStatus) => {
    try {
      const response = await fetch(
        `http://localhost:3000/room/status/${roomId}`,
        {
          method: "PUT",
  ...
  const handleDelete = async (roomId) => {
    if (confirm("Are you sure you want to delete this room?")) {
      try {
        const response = await fetch(
          `http://localhost:3000/room/delete/${roomId}`,
          {
            method: "DELETE",
          }
        );
              ...
              <div className="relative h-40 bg-gradient-to-br from-teal-100 to-blue-100 flex items-center justify-center">
                {room.images && room.images.length > 0 ? (
                  ...
                  <div className="text-center">
                    <div className="text-5xl text-teal-300 font-bold">
                      {room.roomNumber}
                    </div>
                  </div>
                )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-900/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <motion.div
            ...
            className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full p-8 space-y-6 max-h-[90vh] overflow-y-auto"
          >
            {/* Modal Header */}
            <div className="flex justify-between items-center">
